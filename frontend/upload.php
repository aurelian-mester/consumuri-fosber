...
<head>
    <meta charset="UTF-8">
    <title>Upload Fișiere Fosber</title>
    <style>
...
<div class="container">
    ...
    <p class="desc">Baza de date va sparge automat câmpurile multiple (cu $|) pe coloane individuale în structura reporting.</p>
    
    <div id="status-message"></div>

    <form id="uploadForm">
        <div class="form-group">
            <label>1. Alege Tipul de Fișier:</label>
        ...
        <div class="form-group">
            <label>2. Selectează fișierul (.csv):</label>
            <input type="file" name="csv_file" id="csv_file" accept=".csv" required>
        </div>

        <button type="button" onclick="submitUpload()">Încărcare Fișier</button>
    </form>
</div>
...
<script>
    async function submitUpload() {
        ...
        if (fileInput.files.length === 0) {
            alert("Vă rugăm să selectați un fișier CSV!");
            return;
        }

        // Afisare mesaj de Incarcare
        msgBox.className = "loading";
        msgBox.innerHTML = "⏳ Se procesează și se curăță fișierul... Așteptați...";
        msgBox.style.display = "block";

        // Cream formularul pentru a trimite fisierul la Go
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                fileInput.value = ""; // Golim input-ul
            } else {
